...
class FlexibleMobilizationImport implements ToModel, WithHeadingRow, WithValidation, SkipsEmptyRows
{
    use Importable;
    ...
    private function getHeaderMapping()
    {
        return [
            ...
            'whatsapp_number' => ['whatsapp_number', 'whatsapp number', 'whatsapp', 'whatsapp_no', 'whatsapp no'],
    ...
    public function model(array $row)
{
    ...
    // ✅ STRONG EMPTY ROW CHECK (FIRST THING)
    if (
    ...
    // ✅ Fallback if headers not matched
    if (empty($name) || empty($email) || empty($mobile)) {
        $values = array_values(array_filter($row, function ($val) {
            return !is_null($val) && trim((string)$val) !== '';
        }));

        if (empty($name) && isset($values[0])) {
            $name = trim((string) $values[0]);
        }

        if (empty($email) && isset($values[1])) {
            $email = trim((string) $values[1]);
        }

        if (empty($mobile) && isset($values[2])) {
            $mobile = trim((string) $values[2]);
        }
    }
    ...
    return new Mobilization([
        ...
        'email' => $email,
        'mobile' => $mobile,
        'whatsapp_number' => $this->getValue($row, 'whatsapp_number'),
