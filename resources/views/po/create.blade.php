...
<div style="padding:10px; width:100%; display:flex; flex-direction:column; align-items:center;">
    ...
    <form action="{{ route('po.store') }}" method="POST"
        ...
        <div style="margin-bottom:20px;">
            <label style="font-weight:600; color:#6366f1;">PO / WO Number <span style="color:#ef4444;">*</span></label>
            <input type="text" name="po_no" value="{{ old('po_no') }}" required placeholder="Enter PO/WO Number..."
                style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(99,102,241,0.3); background:#fff;">
        </div>

        {{-- Date --}}
        <div style="margin-bottom:20px;">
            <label style="font-weight:600; color:#6366f1;">Date <span style="color:#ef4444;">*</span></label>
            <input type="date" name="po_date" value="{{ old('po_date') }}" required
                style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(99,102,241,0.3);">
        </div>

        {{-- Period From --}}
        <div style="margin-bottom:20px;">
            <label style="font-weight:600; color:#6366f1;">Period From <span style="color:#ef4444;">*</span></label>
            <input type="date" name="period_from" value="{{ old('period_from') }}" required
                style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(99,102,241,0.3);">
        </div>

        {{-- Period To --}}
        <div style="margin-bottom:20px;">
            <label style="font-weight:600; color:#6366f1;">Period To <span style="color:#ef4444;">*</span></label>
            <input type="date" name="period_to" value="{{ old('period_to') }}" required
                style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(99,102,241,0.3);">
        </div>
        ...
        <div style="margin-bottom:20px;">
            ...
            <input type="number" step="0.01" name="gst" value="{{ old('gst') }}" placeholder="Enter GST..."
                style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(99,102,241,0.3);">
        </div>
