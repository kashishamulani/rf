...
<div class="page-wrap">
    ...
    <form id="filterForm" class="mobi_filter" method="GET" action="{{ route('mobilizations.index') }}"
        ...
        <div style="display: flex; gap: 8px; align-items: center; padding-bottom: 2px; margin-top: 10px;">
                ...
                <tr style="border-bottom:1px solid #f1f5f9;">
                    ...
                    <td style="padding:6px;">
                        <div style="display:flex; align-items:center; gap:6px;">
                            @if($m->form_responses_count > 0 || ($m->latestFormResponse &&
    ...
    fetch('/mobilizations/' + mobilizationId + '/form-files')
        ...
        .then(function(data) {
            ...
            if (data.success && data.files && data.files.length > 0) {
                ...
                alert('No files available to download for this candidate.');
            } else {
                alert('Error: ' + (data.message || 'Failed to load files'));
            }
        })
...
function showFilesModal(files, mobilizationName) {
    var existingModal = document.getElementById('filesModal');
    if (existingModal) existingModal.remove();

    // Store files globally for ZIP download
    window.currentFilesForZip = files;

    var modalHtml = '<div id="filesModal" class="modal" style="display:flex;">' +
        '<div class="modal-box" style="max-width: 650px;">' +
        '<h3 style="margin-bottom: 10px;">📁 Download Files - ' + escapeHtml(mobilizationName) + '</h3>' +
        '<p style="margin-bottom: 20px; color: #6b7280;">Select files to download individually or download all as ZIP</p>' +
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        modalHtml +=
            ...
            '<div style="flex: 1;">' +
            '<div style="font-weight: 600; font-size: 14px; margin-bottom: 4px;">' + escapeHtml(file.field_label) +
            '</div>' +
            '<div style="font-size: 12px; color: #6b7280;"><i class="fa-solid fa-file"></i> ' + escapeHtml(file
                .file_name) + '</div>' +
            '</div>' +
            '<div style="display: flex; gap: 8px;">' +
            '<button onclick="downloadSingleFile(\'' + escapeJsString(file.file_url) + '\', \'' + escapeJsString(file
                .file_name) +
    ...
    modalHtml += '</div>' +
        ...
        '<button onclick="closeFilesModal()" class="btn btn-cancel" style="padding: 8px 16px; background: #e5e7eb; color: #111827; border: none; border-radius: 6px; cursor: pointer;">Close</button>' +
        '<button onclick="downloadAllFilesAsZip()" style="padding: 8px 20px; background: linear-gradient(135deg, #16a34a, #22c55e); color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600;">' +
        '<i class="fa-solid fa-file-zipper"></i> Download All as ZIP</button>' +
        '</div></div></div>';
...
function downloadSingleFile(fileUrl, fileName) {
    ...
    var downloadUrl = '/document/' + encodeStoragePath(fileUrl);
...
async function downloadAllFilesAsZip() {
    var fileUrls = window.currentFilesForZip ? window.currentFilesForZip.map(function(f) {
        return f.file_url;
    }) : [];
    ...
    try {
        var response = await fetch('/mobilizations/download-files-zip', {
            method: 'POST',
            headers: {
        ...
        a.href = url;
        a.download = 'form_files_' + Date.now() + '.zip';
...
var currentMobilizationId = null;
...
function showLoadingModal() {
    var existingModal = document.getElementById('loadingModal');
    if (existingModal) existingModal.remove();

    var modalHtml = '<div id="loadingModal" class="modal" style="display:flex;">' +
...
function showFormSelectionModal(mobilizationId, data) {
    var existingModal = document.getElementById('formSelectionModal');
    if (existingModal) existingModal.remove();

    if (!data.has_assignments) {
        var modalHtml = '<div id="formSelectionModal" class="modal" style="display:flex;">' +
            '<div class="modal-box" style="max-width: 500px; text-align: center;">' +
            '<div style="margin-bottom: 20px;"><i class="fa-solid fa-circle-exclamation" style="font-size: 48px; color: #f59e0b;"></i></div>' +
            '<h3 style="margin-bottom: 10px; color: #374151;">No Assignment Assigned Yet</h3>' +
            '<p style="margin-bottom: 20px; color: #6b7280;">This student is not assigned to any assignment yet.<br>Please assign the student to an assignment first before sending forms.</p>' +
            '<div class="modal-actions" style="justify-content: center;">' +
            '<button onclick="closeFormSelectionModal()" class="btn btn-cancel" style="padding: 10px 24px; background: #e5e7eb; border: none; border-radius: 6px; cursor: pointer;">Close</button>' +
            '</div></div></div>';
        document.body.insertAdjacentHTML('beforeend', modalHtml);
        return;
    }
    ...
    if (!data.has_forms) {
        var modalHtml = '<div id="formSelectionModal" class="modal" style="display:flex;">' +
            '<div class="modal-box" style="max-width: 500px; text-align: center;">' +
            '<div style="margin-bottom: 20px;"><i class="fa-solid fa-file-circle-xmark" style="font-size: 48px; color: #9ca3af;"></i></div>' +
            '<h3 style="margin-bottom: 10px; color: #374151;">No Forms Available</h3>' +
            '<p style="margin-bottom: 20px; color: #6b7280;">' + escapeHtml(data.message ||
                'No forms are linked to the assignments this student is assigned to.') + '</p>' +
            '<div class="modal-actions" style="justify-content: center;">' +
            '<button onclick="closeFormSelectionModal()" class="btn btn-cancel" style="padding: 10px 24px; background: #e5e7eb; border: none; border-radius: 6px; cursor: pointer;">Close</button>' +
            '</div></div></div>';
        document.body.insertAdjacentHTML('beforeend', modalHtml);
    ...
    for (var i = 0; i < forms.length; i++) {
        ...
        formsListHtml +=
            ...
            (form.description ? '<p style="margin-top: 5px; color: #6b7280; font-size: 13px;">' + escapeHtml(form
                .description) + '</p>' : '') +
            '</div>' +
            '<button onclick="generateFormLink(' + mobilizationId + ', ' + form.id +
            ')" style="padding: 8px 16px; background: linear-gradient(135deg,#6366f1,#ec4899); color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; margin-left: 10px;">Generate Link</button>' +
            '</div></div></div>';
    }
    ...
    var modalHtml = '<div id="formSelectionModal" class="modal" style="display:flex;">' +
        ...
        '<p style="margin-bottom: 20px; color: #6b7280;">Choose a form to generate a prefilled link for this candidate<br>' +
        '<small style="color: #6366f1;">Showing forms linked to this student\'s assignments</small></p>' +
        '<div id="formsList" style="max-height: 400px; overflow-y: auto;">' + formsListHtml + '</div>' +
        '<div class="modal-actions" style="margin-top: 20px;">' +
        '<button onclick="closeFormSelectionModal()" class="btn btn-cancel" style="padding: 8px 16px; background: #e5e7eb; border: none; border-radius: 6px; cursor: pointer;">Close</button>' +
        '</div></div></div>';
...
function closeFormSelectionModal() {
    var modal = document.getElementById('formSelectionModal');
    if (modal) modal.remove();
}
...
function showLinkModal(link, candidateName, formTitle) {
    var modalHtml = '<div id="linkModal" class="modal" style="display:flex;">' +
        '<div class="modal-box" style="max-width: 550px;">' +
        '<h3 style="margin-bottom: 15px;">🔗 Form Link Generated</h3>' +
        '<p style="margin-bottom: 10px; color: #374151;">' +
        '<strong>Candidate:</strong> ' + escapeHtml(candidateName) + '<br>' +
        '<strong>Form:</strong> ' + escapeHtml(formTitle) + '</p>' +
        '<div style="background: #f3f4f6; padding: 12px; border-radius: 8px; margin: 15px 0;">' +
        '<label style="font-size: 12px; color: #6b7280; margin-bottom: 5px; display: block;">Share this link with the candidate:</label>' +
        '<div style="display: flex; gap: 8px;">' +
        '<input type="text" id="formLink" value="' + escapeHtml(link) +
        '" readonly style="flex: 1; padding: 8px; border: 1px solid #e5e7eb; border-radius: 6px; background: white; font-size: 12px;">' +
        '<button onclick="copyLink()" style="padding: 8px 12px; background: #6366f1; color: white; border: none; border-radius: 6px; cursor: pointer;">Copy</button>' +
        '</div></div>' +
        '<div style="margin-bottom: 15px;">' +
        '<label style="font-size: 13px; font-weight: 600; margin-bottom: 8px; display: block;">Or send via:</label>' +
        '<div style="display: flex; gap: 10px;">' +
        '<button onclick="sendViaWhatsApp(\'' + escapeJsString(link) + '\', \'' + escapeJsString(candidateName) +
        '\')" style="flex: 1; padding: 8px; background: #25D366; color: white; border: none; border-radius: 6px; cursor: pointer;">📱 WhatsApp</button>' +
        '<button onclick="sendViaEmail(\'' + escapeJsString(link) + '\', \'' + escapeJsString(candidateName) +
        '\')" style="flex: 1; padding: 8px; background: #EA4335; color: white; border: none; border-radius: 6px; cursor: pointer;">📧 Email</button>' +
        '<button onclick="sendViaSMS(\'' + escapeJsString(link) + '\', \'' + escapeJsString(candidateName) +
        '\')" style="flex: 1; padding: 8px; background: #34B7F1; color: white; border: none; border-radius: 6px; cursor: pointer;">💬 SMS</button>' +
        '</div></div>' +
        '<div class="modal-actions" style="margin-top: 20px;">' +
        '<button onclick="closeLinkModal()" class="btn btn-cancel" style="padding: 8px 16px; background: #e5e7eb; border: none; border-radius: 6px; cursor: pointer;">Close</button>' +
        '</div></div></div>';
    document.body.insertAdjacentHTML('beforeend', modalHtml);
}
    ...
    fetch('/mobilizations/get-contact/' + getCurrentMobilizationId())
        ...
        .then(function(data) {
            if (data.whatsapp_number) {
                var message = encodeURIComponent('Dear ' + candidateName + ',\n\nPlease fill out this form: ' +
                    link + '\n\nThank you!');
...
function sendViaEmail(link, candidateName) {
    fetch('/mobilizations/get-contact/' + getCurrentMobilizationId())
        ...
        .then(function(data) {
            if (data.email) {
                var subject = encodeURIComponent('Form Submission Request - ' + candidateName);
                var body = encodeURIComponent('Dear ' + candidateName +
...
function sendViaSMS(link, candidateName) {
    fetch('/mobilizations/get-contact/' + getCurrentMobilizationId())
        ...
        .then(function(data) {
            if (data.mobile) {
                var message = encodeURIComponent('Dear ' + candidateName + ', please fill this form: ' + link);
...
function showFormDataModal(formData, candidateName, formTitle) {
    ...
    for (var i = 0; i < formData.length; i++) {
        ...
        itemsHtml +=
            ...
            escapeHtml(item.field_name) + '</div>' +
            '<div style="word-break: break-word;">';
        if (item.file_url) {
            itemsHtml += '<a href="/document/' + encodeStoragePath(item.file_url) +
                '" target="_blank" style="display: inline-flex; align-items: center; gap: 6px; color: #6366f1; text-decoration: none; font-weight: 500; padding: 8px 12px; background: #e0e7ff; border-radius: 6px;">' +
                '<i class="fa-solid fa-download"></i> <span>' + (item.file_type && item.file_type.includes('image') ?
                    'View Image' : 'Download File') + '</span></a>';
        } else {
            itemsHtml += '<p style="margin: 0; color: #374151; font-size: 14px; line-height: 1.5;">' + escapeHtml(item
                .value || '-') + '</p>';
        }
        itemsHtml += '</div></div>';
    }

    var modalHtml = '<div id="formDataModal" class="modal" style="display:flex;">' +
        ...
        '<div><h3 style="margin: 0; font-size: 18px; color: #111;">📋 ' + escapeHtml(formTitle || 'Form Data') +
        '</h3>' +
        '<p style="margin: 5px 0 0 0; color: #6b7280; font-size: 13px;">Candidate: <strong>' + escapeHtml(
            candidateName) + '</strong></p></div></div>' +
        '<div style="margin-bottom: 20px;"><div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 15px;">' +
        itemsHtml + '</div></div>' +
        ...
        '<button onclick="closeFormDataModal()" style="padding: 10px 20px; background: #f3f4f6; color: #374151; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">Close</button></div></div></div>';
    document.body.insertAdjacentHTML('beforeend', modalHtml);
}

function closeFormDataModal() {
    var modal = document.getElementById('formDataModal');
    if (modal) modal.remove();
}
...
function showAssignments(id) {
    var html = document.getElementById('assignments-' + id).innerHTML;
    document.getElementById('assignmentList').innerHTML = html;
    document.getElementById('assignmentModal').style.display = 'flex';
}
...
function openRemarkModal(id) {
    document.getElementById('remarkModal').style.display = 'flex';
    document.getElementById('remarkForm').action = '/mobilizations/' + id + '/remark';
    fetch('/mobilizations/' + id + '/remarks')
        .then(function(res) {
            return res.json();
        })
        .then(function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var r = data[i];
                html += '<div style="border-bottom:1px solid #eee;padding:6px 0;"><div>' + r.remark + '</div>' +
                    '<small style="color:#9ca3af;">' + new Date(r.created_at).toLocaleString() + '</small></div>';
            }
            document.getElementById('remarkHistory').innerHTML = html;
        });
}
