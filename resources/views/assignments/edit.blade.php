...
.assignment-card{width:100%; max-width:1150px; background:#fff; padding:32px 40px; border-radius:18px; box-shadow:0 20px 40px rgba(0,0,0,0.08);}
.form-header{display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;}
.form-header h2{font-size:26px; font-weight:700; color:#4f46e5;}
.back-btn{padding:10px 18px; background:#f1f5f9; color:#4f46e5; border-radius:10px; font-weight:600; text-decoration:none;}
.form-grid{display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:20px;}
.form-group label{font-weight:600; display:block; margin-bottom:5px; color:#4338ca;}
.form-control{width:100%; padding:12px; border-radius:10px; border:1px solid #e5e7eb;}
.section{margin-top:35px;}
.hr-card{margin-top:12px; padding:14px; border-radius:12px; background:#f0fdf4;}
.submit-btn{margin-top:40px; padding:16px; width:100%; font-size:16px; background:linear-gradient(135deg,#6366f1,#ec4899); color:#fff; font-weight:700; border-radius:14px; border:none;}
.modal-overlay{display:none; position:fixed; inset:0; background:rgba(0,0,0,0.4); justify-content:center; align-items:center; z-index:9999;}
.modal-box{background:#fff; width:500px; max-height:80vh; overflow:auto; border-radius:14px; padding:20px;}
.hr-item{padding:10px; border-bottom:1px solid #eee; cursor:pointer;}
.hr-item:hover{background:#f3f4f6;}
</style>
...
<option value="Attrition" {{ $assignment->batch_type=='Attrition'?'selected':'' }}>Attrition</option>
</select>
</div>
...
<button type="button" onclick="openHrModal()" class="back-btn" style="background:#22c55e;color:#fff;">➕ Select HR</button>
...
<div class="form-grid">
<input type="text" name="sm_name" class="form-control" placeholder="Name" value="{{ $assignment->sm_name }}">
<input type="text" name="sm_mobile" class="form-control" placeholder="Mobile" value="{{ $assignment->sm_mobile }}">
<input type="email" name="sm_email" class="form-control" placeholder="Email" value="{{ $assignment->sm_email }}">
<input type="text" name="store_code" class="form-control" placeholder="Store Code" value="{{ $assignment->store_code }}">
</div>
</div>
...
<div id="hrList"></div>
<button onclick="closeHrModal()" class="back-btn" style="background:#ef4444;color:#fff;margin-top:10px;">Close</button>
</div>
</div>
...
const oldDistrict="{{ $assignment->district }}";

fetch('/states').then(r=>r.json()).then(data=>{
...
o.dataset.code=st.iso2;
if(st.name===oldState) o.selected=true;
s.appendChild(o);
});
loadDistricts();
});
...
let s=document.getElementById('state');
let code=s.options[s.selectedIndex]?.dataset.code;
if(!code) return;
fetch('/districts/'+code).then(r=>r.json()).then(data=>{
let d=document.getElementById('district');
d.innerHTML='<option value="">Select District</option>';
data.forEach(di=>{
let o=document.createElement('option');
o.value=di.name;
o.textContent=di.name;
if(di.name===oldDistrict) o.selected=true;
d.appendChild(o);
});
});
}
