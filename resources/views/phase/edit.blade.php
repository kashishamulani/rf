...
<div style="padding:10px; width:100%; display:flex; flex-direction:column; align-items:center;">
    ...
    <div style="width:100%; max-width:700px; display:flex; justify-content:flex-end; margin-bottom:16px;">
        <a href="{{ route('phase.index') }}" style="
            padding:10px 18px;
            background:#e5e7eb;
            color:#374151;
            font-weight:600;
    ...
    <form action="{{ route('phase.update', $phase->id) }}" method="POST"
        ...
        <div style="margin-bottom:20px;">
            <label style="font-weight:600; color:#6366f1;">Phase Name <span style="color:#ef4444;">*</span></label>
            <input type="text" name="phase_name"
                   value="{{ old('phase_name', $phase->phase_name) }}" required
        ...
        {{-- Phase Order --}}
        <div style="margin-bottom:20px;">
            <label style="font-weight:600; color:#6366f1;">Phase Order <span style="color:#ef4444;">*</span></label>
            <select name="phase_order" required
                style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(99,102,241,0.3); background:#fff;">
                <option value="">Select Order</option>
                @for($i=1; $i<=20; $i++)
                    <option value="{{ $i }}" {{ old('phase_order', $phase->phase_order) == $i ? 'selected' : '' }}>
                        {{ $i }}
                    </option>
                @endfor
            </select>
